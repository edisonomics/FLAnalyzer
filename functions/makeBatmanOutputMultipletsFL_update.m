...
for i = 1:size(basis,2)
    ...
    % Estimate noise floor (usually the lower-amplitude regions)
    % Option A: Use all data and estimate background
    noiseFloor = mean(norm_base_2) + std(norm_base_2);

    % Set adaptive minpeakheight as a multiple of noise floor
    % Adjust the multiplier (e.g., 2-5x) based on your data
    PeakHmin = 3 * noiseFloor;
    [pks,locs] = findpeaks(norm_base_2,"MinPeakHeight",PeakHmin);
    if isempty(pks)
        continue
    end
...
indexer = 1:length(metabolites);
indexer = indexer(metabolites == "");
metabolites(indexer) = [];
