...
for i = 1:size(all_peak_tables,2)
    if ~isempty(all_peak_tables(i).ft_peaks)
        peaks_idx = i;
        break
    end
end
...
sum_over_fractions = zeros(1,length(all_peak_tables(peaks_idx).ft_peaks(1).ft));
for i = 1:size(all_peak_tables,2)
...
X_removed = zeros(size(X,1),length(all_peak_tables(peaks_idx).ft_peaks(1).ft));
...
for i = 1:size(all_peak_tables_removed,2)
    ...
    X_loop = zeros(size(all_peak_tables_removed(i).ft_peaks,2),length(all_peak_tables(peaks_idx).ft_peaks(1).ft));
    for j = 1:size(X_loop,1)
