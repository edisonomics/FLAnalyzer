function annotation_fraction_plot(X,ppm,isbackground,database_gissmo,annotate_matrix,delete_annotation,delete_width)

%{
    Chris Esselman 5.7.24
    
    annotation_fraction_plot
        Create a 3D stacked plot of fraction libraries
    ...
    ppm_range       range of ppm to show ex/ [1 3]

    zLIM            Change the intensity level ex/ [0 10]
...
colors = winter(rows);
fig = figure('Position',[10 10 900 750]);
ax1 = axes(fig);
hold on
%Plot each fraction
if ~delete_annotation && ~isbackground
    for i = 1:rows - 1
        plot3(x(i,:),z(i,:),y(i,:),'Color',colors(i,:),'LineWidth',1.25)
    end
    plot3(x(rows,:),z(rows,:),y(rows,:),'Color',colors(i,:),'LineWidth',1.25)
elseif ~delete_annotation && isbackground
    for i = 1:rows - 1
        plot3(x(i,:),z(i,:),y(i,:),'Color',colors(i,:),'LineWidth',1.25)
    end
    plot3(x(rows,:),z(rows,:),y(rows,:),'Color',"k",'LineWidth',1.25)
end
% Do the annotation
